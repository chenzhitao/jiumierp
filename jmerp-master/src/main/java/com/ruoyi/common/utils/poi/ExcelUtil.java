    ...
    public Cell addCell(Excel attr, Row row, T vo, Field field, int column)
    {
        ...
        try
        {
            ...
            if (attr.isExport())
            {
				...
				// 税率
				else if (StringUtils.isNotEmpty(taxRate) && StringUtils.isNotNull(value))
                {
                    cell.setCellValue((new BigDecimal(value.toString()).multiply(new BigDecimal(100))).toString() + "%");
                }
                else
                {
                    // 设置列类型
